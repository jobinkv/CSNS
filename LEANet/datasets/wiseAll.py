...
import datasets.wiseAll_labels as wise_labels
import copy
...
labels_name=wise_labels.name2label
...
def make_dataset(quality, mode, maxSkip=0, fine_coarse_mult=6, cv_split=0):
    ...
    # reading cluster centre 
    cluster_filepath = os.path.join('/home/jobinkv/clusterList.txt')
    if not os.path.exists(cluster_filepath):
        raise Warning("file not existing : ",cluster_filepath)
    clusters = [{"name":id.split(',')[0].strip(),"cntr":int(id.split(',')[1].strip())} for id in open(cluster_filepath)]

    def getCluster(imageName):
        selected = list(filter(lambda item: item["name"] == imageName, clusters))
        if len(selected)>0:
            return selected[0]["cntr"]-1
        else:
...
if __name__ == '__main__':
    ...
    dataloader = WiseDataWithPos('fine','trainval',0,transform=train_input_transform)
    st()
    data = dataloader[0]
    print ('yahooo')
