...
import datasets.spaseAll_labels as spase_labels
import copy
import ipdb
...
trainid_to_name = spase_labels.trainId2name
id_to_trainid = spase_labels.label2trainid
labels_name=spase_labels.name2label
labels_name_list = list(labels_name.keys())
num_classes = len(labels_name_list)
ignore_label = 255
root = cfg.DATASET.SPASE_DIR
img_postfix = '.png'
...
def make_dataset(quality, mode, maxSkip=0, fine_coarse_mult=6, cv_split=0):
    ...
    if quality == 'coarse':
        ...
        for mod in modes:
            if mod == 'test':
                ...
                for id in open(item_list_filepath):
                    ...
                    (os.path.join(root, "labels",id.strip())),getCluster(id.strip().replace('.mat','.jpg')))
                    items.append(item)
                logging.info('{} fine spase: '.format(mod) + str(len(items)))
            else:
                item_list_filepath = os.path.join(root, mod+".txt")
                if not os.path.exists(item_list_filepath):
                    raise Warning("file not existing : ",item_list_filepath)
        ...
        raise 'unknown spase quality {}'.format(quality)
    # logging.info('Cityscapes-{}: {} images'.format(mode, len(items)))
    logging.info('Spase-{}: {} images'.format(mode, len(items) + len(aug_items)))
...
class SpaseData(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        mat = hdf5storage.loadmat(mask_path)
        label = np.array(mat['labels'])
        mask = np.transpose(label,(2,0,1))
        #mask = mask[1:26,:,:]# removing the zeroth layer 
        mask[mask>=1] = 1
        ...
        if self.transform is not None:
            img = self.transform(img)
             #img = self.transflf.target_aux_transform is not None:
        ...
        mask_aux = torch.from_numpy(mask)
        print ('spaseAll called')
        # Debug
        if self.dump_images:
...
class SpaseDataVideo(data.Dataset):
...
class SpaseDataUniform(data.Dataset):
    ...
    def __init__(self, quality, mode, maxSkip=0, joint_transform_list=None, sliding_crop=None,
        ...
        json_fn = 'spase_{}_cv{}_tile{}.json'.format(
        ...
        if self.maxSkip > 0:
            json_fn = 'spase_{}_cv{}_tile{}_skip{}.json'.format(
        ...
        if self.coarse_boost_classes is not None:
            json_fn = 'spase_coarse_{}_tile{}.json'.format(
...
class SpaseDataUniformWithPos(data.Dataset):
    
    def __init__(self, quality, mode, maxSkip=0, joint_transform_list=None, sliding_crop=None,
                 transform=None, target_transform=None, target_aux_transform=None, dump_images=False,
                 cv_split=None, class_uniform_pct=0.5, class_uniform_tile=1024,
                 test=False, coarse_boost_classes=None, pos_rfactor=8):
        print("######## SpaseDataUniformWithPos #########")
        self.quality = quality
        self.mode = mode
        self.maxSkip = maxSkip
        self.joint_transform_list = joint_transform_list
        ...
        # Add in coarse centroids for certain classes
        if self.coarse_boost_classes is not None:
            json_fn = 'spase_{}_tile{}.json'.format(
                self.mode, self.class_uniform_tile)
            if os.path.isfile(json_fn):
                with open(json_fn, 'r') as json_data:
                    centroids = json.load(json_data)
...
class SpaseDataWithPos(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        img_path, mask_path,cntr = self.imgs[index]
        ...
        img = F.interpolate(img, (540,720), mode='bilinear',align_corners=True)
        mask = F.interpolate(mask, (540,720), mode='nearest')
        ...
        return torch.squeeze(img), torch.squeeze(mask), img_name, torch.squeeze(mask), (pos_h, pos_w),cntr
...
if __name__ == '__main__':
    ...
    dataloader = SpaseDataWithPos('fine','trainval',0,transform=train_input_transform)
    ipdb.set_trace()
    data = dataloader[0]
    print ('yahooo')
