...
class PosEmbedding1D(nn.Module):
    ...
    def __init__(self, pos_rfactor, dim, pos_noise=0.0):
        ...
        self.sel_index = torch.tensor([0]).cuda()
        self.pos_layer = nn.Embedding((128//pos_rfactor*2)+1, dim)
    ...
    def forward(self, x, pos, return_posmap=False):
        ...
        if self.visualise_pos:
            ...
            temp1 = np.uint8(temp1.detach().cpu()*255)
            tmp = cv2.applyColorMap(temp1, cv2.COLORMAP_JET)   
            cv2.imwrite('pos_embedding_cheeting.jpg',tmp)
            st()
        if return_posmap:
            return x, self.pos_layer.weight # 33 X 80
        return x
...
class PosEncoding1Dw(nn.Module):
    
    def __init__(self, pos_rfactor, dim, pos_noise=0.0):
        super(PosEncoding1Dw, self).__init__()
        print("use PosEncoding1Dw")
        self.sel_index = torch.tensor([0]).cuda()
        pos_enc = (get_sinusoid_encoding_table((128//pos_rfactor)+1, dim) + 1)
        self.pos_layer = nn.Embedding.from_pretrained(embeddings=pos_enc, freeze=True)
        self.pos_noise = pos_noise
    ...
    def forward(self, x, pos, return_posmap=False):
        ...
        if self.visualise_pos:
            ...
            temp1 = np.uint8(temp1.detach().cpu()*255)
            tmp = cv2.applyColorMap(temp1, cv2.COLORMAP_JET)   
            cv2.imwrite('pos_w_cheeting.jpg',tmp)
        if return_posmap:
            return x, self.pos_layer.weight # 33 X 80
        return x

class PosEncodingOnly(nn.Module):
    
    def __init__(self, pos_rfactor, dim, pos_noise=0.0):
        super(PosEncodingOnly, self).__init__()
        print("use PosEncoding1D")
        self.sel_index = torch.tensor([0]).cuda()
        pos_enc = (get_sinusoid_encoding_table((128//pos_rfactor)+1, dim) + 1)
        self.pos_layer = nn.Embedding.from_pretrained(embeddings=pos_enc, freeze=True)
    ...
    def forward(self,pos, return_posmap=False):
        pos_h, _ = pos # B X H X W
        pos_h = pos_h//self.pos_rfactor
        pos_h = pos_h.index_select(2, self.sel_index).unsqueeze(1).squeeze(3) # B X 1 X H
        pos_h = nn.functional.interpolate(pos_h.float(), size=128//self.pos_rfactor, mode='nearest').long() # B X 1 X 48
        ...
        return self.pos_layer(pos_h).transpose(1,3).squeeze(3)   # B X 1 X 48 X 80 > B X 80 X 48 X 1 
