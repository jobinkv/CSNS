...
def get_loss(args):
    ...
    if args.cls_wt_loss:
        ce_weight = None #torch.Tensor([0.8373, 0.9180, 0.8660, 1.0345, 1.0166, 0.9969, 0.9754,
    ...
    criterion_val = nn.BCEWithLogitsLoss(reduction='mean').cuda()
    return criterion, criterion_val,criterion_cluster
...
def get_loss_aux(args):
    ...
    print("auxilary loss with binary cross entropy with logistsLoss")
    criterion = nn.BCEWithLogitsLoss(reduction='mean').cuda()
    #criterion = nn.CrossEntropyLoss(weight=ce_weight, reduction='mean',
    #                                ignore_index=args.dataset_cls.ignore_label).cuda()
...
def get_loss_bcelogit(args):
    if args.cls_wt_loss:
        pos_weight = None #torch.Tensor([0.8373, 0.9180, 0.8660, 1.0345, 1.0166, 0.9969, 0.9754,
                          #      1.0489, 0.8786, 1.0023, 0.9539, 0.9843, 1.1116, 0.9037,
                          #      1.0865, 1.0955, 1.0865, 1.1529, 1.0507])
    else:
        pos_weight = None
