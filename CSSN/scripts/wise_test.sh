...
cp /home/jobinkv/clusterList.txt /ssd_scratch/cvit/jobinkv/
dataset='wiseAll' #'spase' #'wise' 
attn='LEANetv1'
...
rsync -avz 10.2.16.142:/mnt/3/icdar21/slide/result4paper/$model_name /ssd_scratch/cvit/jobinkv/$SLURM_JOB_ID/ 
...
python trainslide.py --dataset $dataset\
  ...
  --city_mode $mode  --lr 0.03 --poly_exp 0.9 \
  --hanet_lr 0.03 --hanet_poly_exp 0.9 \
  --crop_size 564  --color_aug 0.25  --max_iter 27000  \
  --bs_mult 2 --pos_rfactor 9 --dropout 0.1  \
  ...
  --template_selection_loss_contri 0.1 --backbone_lr 0.02 --multi_optim --test_mode True

rsync -avz /ssd_scratch/cvit/jobinkv/$SLURM_JOB_ID/$SLURM_JOB_ID 10.2.16.142:/mnt/1/pytorchTensorboard/runs/visual4paper/$dataset-$attn-$model
